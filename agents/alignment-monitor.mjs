 ...
 * @param {object} [opts]
 * @param {boolean} [opts.dryRun] - Preview the report without writing it.
 * @returns {object} findings ({score, driftAlerts, promptSuggestions, ...})
 */
export function runAlignmentCheck({ dryRun = false } = {}) {
  ...
  if (dryRun) {
    ...
    logCapabilityUsage('alignmentMonitor', 'system', 'alignment-check', 'alignment-monitor.mjs', 'check');
  }
...
if (__isMainModule) {
  ...
  if (args.includes('--report')) {
    ...
    runAlignmentCheck({ dryRun: args.includes('--dry-run') });
  }
}
