...
- **We have internal debt** that undercuts our own moats: the self-improvement loop is *advisory, not enforcing* (the review hook can only warn, never fail; pattern-hunt's generic path emits always-passing tests; the alignment score is hand-tuned magic numbers); CI runs only 2 of ~22 test files; token/cost accounting is `chars/4` estimates; and there are latent bugs + doc/no-OpenAI drift.
...
- Internal audit: `/home/bryce/agentic-sdlc` — 40 scripts (≈31 solid, ≈6 partial, 0 pure stubs), 9 real LLM adapters, real local embeddings + AST validation. Verified gaps: `review-hook.mjs` can't fail; `pattern-hunt.mjs:776` TODO always-passing scaffold; `model-manager` `research` is a no-op; `cost-tracker` computes tokens not `$`; `daily-review.mjs` has dead code (unimported `fs`/`path`); CI (`.github/workflows`) runs only `adapter-and-model-manager` + `test-behavior`; `model-intel.default.json` is Anthropic/OpenAI-centric (contradicts the no-OpenAI budget); several scripts miss the mandated `__isMainModule` guard.
...
- Constraints (ours): **zero npm deps**, **privacy-first / no-OpenAI**, self-hostable-preferred, every module tested, `__isMainModule`-guarded, provider-neutral adapters. The affordable-OpenRouter drain (change `autonomous-drain`) is the execution engine that will consume the tasks this roadmap produces.
...
- **Compounds our moats into durable advantage:** the Self-Improvement Flywheel (P9 = P2 ⇄ pattern-hunt ⇄ maturity) makes "agents get better over time" a measured graph, not a claim — **no competitor can do this**; deterministic-replay parallel attempts (P6) productizes a technique almost nobody ships; exact cost + circuit-breaker (P4) makes "cost-governed autonomous engineering" a real enterprise wedge.
