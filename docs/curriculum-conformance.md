...
| **Error deduplication (hash signature)** | Solid | `agents/notify.mjs` — `errorSignature()` + suppression window, writing a runtime ledger to pm/error-signatures.json (gitignored, so it is named here as prose rather than as an owner path); duplicate triggers are suppressed with a seen-count (PR #23) |
